...
location = np.loadtxt('河北城市坐标.txt')
...
def distance_p2p_mat():
    ...
    for i in range(30):
        dis_mat_each = []
        for j in range(30):
            dis = math.sqrt(pow((location[i][0]-location[j][0])*111, 2)+pow((location[i][1]-location[j][1])*111, 2))
...
while True:
      ...
      if not city_name:
          break
name_file.close()
for i in range(0, 30):  # 标注城市名称
   plt.annotate(s=name[i], xy=(m[i], n[i]), xycoords='data', fontsize=5)
plt.savefig('河北最佳路线图.pdf')
plt.show()
